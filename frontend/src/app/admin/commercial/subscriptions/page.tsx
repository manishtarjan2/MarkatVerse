...
export default function CommercialgtSubscriptionsPage() {
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [formData, setFormData] = useState({ id: '', planName: '', price: 0, status: 'ACTIVE' });
  const [isEditing, setIsEditing] = useState(false);

  const fetchItems = async () => {
    try {
      const res = await fetch('http://localhost:3001/commercial/subscriptions');
      if (res.ok) {
  ...
  const openModal = (item: any = null) => {
    if (item) {
      ...
      setFormData({ id: '', planName: '', price: 0, status: 'ACTIVE' });
      setIsEditing(false);
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = isEditing 
      ? `http://localhost:3001/commercial/subscriptions/${formData.id}`
      : 'http://localhost:3001/commercial/subscriptions';
    const method = isEditing ? 'PATCH' : 'POST';
    const { id, price, ...dataToSend } = formData;
    try {
      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...dataToSend, price: parseFloat(price as any) })
      });
      if (res.ok) {
        fetchItems();
        closeModal();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/commercial/subscriptions/${id}`, { method: 'DELETE' });
  ...
  const handleToggleStatus = async (item: any) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/commercial/subscriptions/${item.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-300 w-full">
      <header className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Commercial &gt; Subscriptions</h1>
          <p className="text-slate-400 mt-2 text-sm">Manage subscription plans and pricing.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {loading ? (
                ...
                <tr><td colSpan={4} className="p-4 text-center text-slate-400">No subscriptions found.</td></tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-700/50 transition-all">
                    <td className="p-4 pl-6 font-bold text-white">{item.planName}</td>
                    <td className="p-4 text-emerald-400 font-bold">${item.price}</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-800">
              <h3 className="text-xl font-bold text-white">{isEditing ? 'Edit Plan' : 'Add Plan'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input required type="text" value={formData.planName} onChange={e => setFormData({...formData, planName: e.target.value})} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all" placeholder="e.g. Pro, Premium" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Price</label>
                <input required type="number" step="0.01" value={formData.price} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all" />
              </div>
              <div>
