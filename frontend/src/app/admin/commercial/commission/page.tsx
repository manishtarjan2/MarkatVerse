...
export default function CommercialgtCommissionPage() {
  ...
  const openModal = (item: any = null) => {
    if (item) {
      ...
      setFormData({ id: '', type: 'CATEGORY', rate: 0, status: 'ACTIVE' });
      setIsEditing(false);
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...dataToSend, rate: parseFloat(rate as any) })
      });
      if (res.ok) {
        fetchItems();
        closeModal();
              ...
              {loading ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-700/50 transition-all">
                    <td className="p-4 pl-6 font-bold text-white">{item.type}</td>
                    <td className="p-4 text-emerald-400 font-bold">{item.rate}%</td>
                    <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Rate (%)</label>
                <input required type="number" step="0.01" value={formData.rate} onChange={e => setFormData({...formData, rate: parseFloat(e.target.value)})} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all" />
              </div>
              <div>
