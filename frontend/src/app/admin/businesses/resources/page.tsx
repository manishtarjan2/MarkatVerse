...
import { ShieldAlert, MonitorPlay, Plus, Trash2, Search, Zap } from 'lucide-react';
...
export default function AdminResourcesPage() {
  ...
  const handleAddResource = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hasEditPermission || !selectedQueue || !newResourceName) return;

    try {
      const res = await fetch(`${API_URL}/service-queue/${selectedQueue.id}/resource`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: newResourceName, type: newResourceType })
      });
      if (res.ok) {
        setNewResourceName('');
        setIsAdding(false);
  ...
  const handleDeleteResource = async (resourceId: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/service-queue/${selectedQueue.id}/resource/${resourceId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        fetchQueues();
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-300 w-full relative">
      <header className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Physical Resources</h1>
          <p className="text-slate-400 mt-2 text-sm">Manage physical resources (rooms, tables, equipment) assigned to storefronts.</p>
        </div>
      </header>
      ...
      {!hasEditPermission && (
        <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-4 rounded-xl flex items-center gap-3 mb-8">
          ...
          <p className="text-sm font-medium">You do not have permission to manage physical resources.</p>
        </div>
      )}
              ...
              {loading && queues.length === 0 ? (
                ...
                filteredQueues.map(queue => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border transition-all ${selectedQueue?.id === queue.id ? 'bg-emerald-500/20 border-emerald-500/50 text-white' : 'bg-slate-900/50 border-slate-700/50 text-slate-300 hover:bg-slate-700/30'}`}
                  >
                    <div className="font-bold">{queue.shopName || 'Unknown Shop'}</div>
                    <div className="text-xs opacity-60 flex items-center gap-2 mt-1">
                      <Zap className="w-3 h-3" /> {queue.resources?.length || 0} Resources
                    </div>
                  </button>
                ))
              )}
        ...
        <div className="lg:col-span-2">
          {selectedQueue ? (
            <div className="bg-slate-800 rounded-2xl border border-slate-700 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-900/50">
                <div>
                  ...
                  <p className="text-sm text-slate-400">Manage physical resources for this queue.</p>
                </div>
                {hasEditPermission && (
                  <button 
                    onClick={() => setIsAdding(!isAdding)}
                    className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center gap-2"
                  >
              ...
              {isAdding && hasEditPermission && (
                <div className="p-6 bg-emerald-500/5 border-b border-emerald-500/20">
                  <form onSubmit={handleAddResource} className="flex gap-4 items-end">
                    <div className="flex-1">
                      <label className="block text-xs font-bold text-slate-400 mb-1.5 uppercase">Resource Identifier</label>
                      <input 
                        type="text" 
                        required
                    ...
                    <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-lg font-bold transition-all h-[42px]">
                      Save
                    </button>
                  </form>
                </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {selectedQueue.resources && selectedQueue.resources.length > 0 ? (
                  selectedQueue.resources.map((res: any) => (
                    <div key={res.id} className="bg-slate-900 p-4 rounded-xl border border-slate-700 flex items-center justify-between group">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-emerald-400 font-medium uppercase tracking-wider mt-0.5">{res.type}</div>
                        </div>
                      </div>
                      {hasEditPermission && (
                        <button 
                  ...
                  <div className="col-span-full py-12 text-center text-slate-500 border-2 border-dashed border-slate-700 rounded-xl">
                    No physical resources assigned to this queue.
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-800 p-12 rounded-2xl border border-slate-700 shadow-sm flex flex-col items-center justify-center text-center h-full min-h-[400px]">
              ...
              <p className="text-slate-400 max-w-sm">Select a storefront or queue from the list on the left to manage its physical resources (tables, rooms, stations).</p>
            </div>
          )}
        </div>
      </div>
