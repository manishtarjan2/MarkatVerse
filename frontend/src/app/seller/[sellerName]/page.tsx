                ...
                <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 sticky top-6">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                      {availableServices.length === 0 && <div className="text-sm text-gray-500 italic p-4">No services available yet.</div>}
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="text-2xl">{service.category === 'Hair' ? '✂️' : service.category === 'Spa' ? '💆' : '✨'}</div>
                              <div>
                  ...
                  <div className="bg-indigo-50/50 rounded-2xl p-5 mb-6">
                        ...
                        {availableServices.filter(s => selectedServices.includes(s.id)).map(service => (
                          <div key={service.id} className="flex justify-between items-center">
                            <div className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                              <span>{service.category === 'Hair' ? '✂️' : '✨'}</span> {service.name}
                            </div>
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-indigo-100/50">
                      ...
                      <div className="text-lg font-black text-indigo-700">
                        ₹{availableServices.filter(s => selectedServices.includes(s.id)).reduce((sum, s) => sum + s.price, 0)}
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    <span>🎟️</span> Pay ₹{availableServices.filter(s => selectedServices.includes(s.id)).reduce((sum, s) => sum + s.price, 0)} & Get Token <span>💳</span>
                  </button>
                ...
                {sellerProducts.map(product => (
                  <div key={product.id} className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group flex flex-col overflow-hidden">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="text-xl font-black text-gray-900 mb-4 mt-auto">
                      ...
                      {product.options && product.options.length > 0 && (
                        <div className="mb-4">
                          <label className="text-xs font-semibold text-gray-500 mb-1 block">Select Option:</label>
                          <select 
                            ...
                            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          >
                      ...
                      <button 
                        ...
                        className="w-full py-2.5 bg-gray-900 hover:bg-indigo-600 text-white font-bold rounded-lg transition-colors duration-300 flex items-center justify-center gap-2"
                      >
